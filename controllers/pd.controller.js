...
module.exports.cropNibirotaFilter=async(req,res)=>{
    if (req.body.saao === "all") {
        ...
        .then(data => {
            res.render('pd/cropNibirota/cropNibirotaTable', {records: data} ,function(err, html) {
                res.send(html);
            });
        })
        .catch(err => {
        ...
        try{
            ...
            res.render('pd/cropNibirota/cropNibirotaTable', { title: 'শস্য নিবিড়তার অগ্রগতির তথ্য',success:'',seventeen: seventeen,eighteen: eighteen,nineteen: nineteen,twenty: twenty,twentyOne: twentyOne,twentyTwo: twentyTwo });
        }
        catch(err){
            console.log("outside",err);
        }
...
module.exports.cropNibirotaDistrictFilter=async(req,res)=>{
    try{
        if (req.body.district === "all") {
            res.send([{id:"all",uname:"all"}]);
                
        }else{
        var upazillas=await upazilla.findAll({where: {dd_id: req.body.district}});
        console.log("inside");
        res.send(upazillas)
        }
...
module.exports.producedCropFilter=async(req,res)=>{
    if (req.body.saao === "all") {
        ...
        .then(data => {
            res.render('pd/producedCrop/producedCropTable', {records: data} ,function(err, html) {
                res.send(html);
            });
        })
        .catch(err => {
...
module.exports.producedCropDistrictFilter=async(req,res)=>{
    try{
        if (req.body.district === "all") {
            res.send([{id:"all",uname:"all"}]);
...
module.exports.selectedFieldFilter=async(req,res)=>{
    if (req.body.saao === "all") {
        await selectedField.findAll({where:{year: req.body.year}})
        .then(data => {
            res.render('pd/selectedField/selectedFieldTable', {records: data} ,function(err, html) {
                res.send(html);
            });
        })
        .catch(err => {
        ...
        await selectedField.findAll({ 
            where: {saao_id: req.body.saao,year: req.body.year}
        })
        .then(data => {
            res.render('pd/selectedField/selectedFieldTable', {records: data} ,function(err, html) {
                res.send(html);
...
module.exports.selectedFieldDistrictFilter=async(req,res)=>{
    try{
        if (req.body.district === "all") {
            res.send([{id:"all",uname:"all"}]);
